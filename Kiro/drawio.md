...
### 参照すること
1. `specs/{cloud}/{project}/design.md`（アーキテクチャ概要・コンポーネント設計）
2. `inputs/{cloud}/{project}/インフラ基本設計書.xlsx`（パラメーター値、AWSの場合）
3. `inputs/{cloud}/{project}/parameter-sheet.xlsx`（パラメーター値、Azure / GCP の場合）
...
| クラウド | 状態 | アイコン形式 |
|---|---|---|
| AWS | ✅ 検証中 | `mxgraph.aws4.*` |
| Azure | ⏳ 検証完了後に追加 | `mxgraph.azure.*` |
| GCP | ⏳ 検証完了後に追加 | `mxgraph.gcp2.*` |
