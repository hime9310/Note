...
| 状態の保持 | グローバル変数(**セッション内限定**) | RITM・確定Mermaid・注釈・抽出結果・判定結果・編集指示(案/確定)・回答回収結果・ナレッジ文案。MS公式仕様ではグローバル変数は1回のセッション(会話)の間だけ保持される。**セッションをまたぎ得るPhase(2・3見直し・4・5・7・9)のトピックは、冒頭で必要な変数が空ならEngから再取得する**(RITMは指示文から、長文の状態はEngがチャット履歴から貼り付け) |
...
MS公式の根拠: 「グローバル変数はAgent内のすべてのトピックで使用でき、1回のユーザーセッションの間有効」「必ず指定どおりに実行させたいものは決定的に扱う(AIプランナーに公開しない、またはユーザー確認を要するトピックで包む)」「トピック内から子Agentへ明示的にリダイレクトでき、子Agentに入力・出力を定義すると出力ごとにトピック変数が自動作成される」「子Agentは常に親の文脈を受け取る」。
...
### 2.5 親のトピック一覧(11本。詳細仕様はInstructions集v6.1。SR=RITMの復帰、PS=長文の状態の復帰の共通部品)
...
| T2 | 再判定 | 「再判断して」 | 子C | 冒頭で変数が空ならEngから受け取る(SR/PS)→提示して終了 |
| T3-1 | アーキテクチャレビュー票作成 | 「レビュー票を作成して」/「レビュー票を見直して」(+指摘) | 子D | 冒頭で変数が空ならEngから受け取る(SR/PS)→提示後 T3-2 へ移動 |
| T3-2 | 編集指示の修正ループ | (T3-1から自動) | 子D | 「質問する」で修正点 or『確定』を待つ。修正なら再呼び出し→自己リダイレクト |
| T4 | 構成図修正案 | 「修正モードを実行して」 | 子E | 冒頭で変数が空ならEngから受け取る(SR/PS)→提示して終了 |
| T5 | 送付用Excel生成 | 「Excelを作成して」 | 子D | 冒頭で変数が空ならEngから受け取る(SR/PS)→提示して終了 |
| T7 | 回答回収 | 「回答を確認して」 | 子D | 冒頭でRITMが空ならEngから受け取る(SR)→提示して終了。**別セッションで始まることが多い** |
| T9 | ナレッジ登録 | 「ナレッジに登録して」 | 子C(2回) | 冒頭で変数が空ならEngから受け取る(SR/PS)→文案提示→「質問する」で『承認』を待つ→登録実行 |
...
親の「保持」はすべてグローバル変数で行う。変数名はInstructions集v6.1のトピック仕様と一致させる。

**寿命の注意**: グローバル変数は**セッションが終わると全て空になる**(MS公式仕様)。Phase 6(利用者の回答待ち)で日をまたぐため、Phase 7以降とPhase 2(ヒアリング待ち)は別セッションで始まる前提。該当トピックは冒頭で「RITMはトリガー入力から(無ければ質問)」「長文の状態(判定結果・確定版編集指示・確定Mermaid・【構成図チェック結果】)は空ならEngがチャット履歴から貼り付け」で復帰する。本番化時はSharePointへの保存・復元ツール(⑨)で置き換える。
...
| 2-3b | 親(T2 SR/PS) | Engから: 指示文 | 別セッションの場合: RITMを指示文から取得(無ければ質問)。Global.DiagramCheck が空なら貼り付けを求める | (空のとき)Engへ: 質問 |
...
| 3-1b | 親(T3-1 SR/PS) | Engから: 指示文 | 別セッションの場合(見直し時に多い): RITMを指示文から取得(無ければ質問)。Global.Judgement が空なら貼り付けを求める | (空のとき)Engへ: 質問 |
...
| 4-2 | 親(T4 SR/PS→エージェントノード) | Engから: 指示文 | RITMを復帰。Global.Mermaid / DiagramCheck / Judgement / EditFinal のうち空のものがあればEngに貼り付けを求めて保存(通常はT3と同じセッションのため発動しない) | 子Eへ: diagramCheck、mermaid、judgement、editFinal |
...
| 5-2 | 親(T5 SR/PS→エージェントノード) | Engから: 指示文 | RITMを復帰。Global.EditFinal が空ならEngに貼り付けを求めて保存 | 子Dへ: mode=Excel生成、ritm、csp=Global.CspFromThread、editInstruction=Global.EditFinal |
...
| 7-2 | 親(T7 SR→エージェントノード) | Engから: 指示文 | **別セッションで始まることが多い**。RITMを指示文から取得(無ければ質問) | 子Dへ: mode=回答回収、ritm |
...
| 9-2 | 親(T9 SR/PS→エージェントノード) | Engから: 指示文 | RITMを復帰。Global.Judgement が空ならEngに貼り付けを求めて保存 | 子Cへ: mode=ナレッジ文案作成、ritm、diagramCheck、judgement=Global.Judgement |
...
| # | 作業 | 状況 |
|---|---|---|
| A1 | **方式検証(最優先)**: ①子Aが親の添付画像を読めるか(子は親の文脈を受け取る仕様。要実機確認) ②子の出力(mermaid等)がトピック変数に入るか ③T1a-2の自己リダイレクトによる改修ループが動くか ④生成オーケストレーションが子を直接呼ばず、トピックを経由するか ⑤長文(Mermaid数百行)が変数経由で劣化なく子Bへ渡るか ⑥**セッション期限の実測**(テストチャット/Teamsで、どれくらい間が空くとグローバル変数が消えるか) | **未実施** |
| A2 | 子A `構成図Mermaid化Agent`: 作成済み。Instructionsは現行のまま。入力・出力を定義(P11) | 作成済み(入出力定義は未) |
| A3 | 子B〜E: Instructions集v6.1 §2〜5を貼付。入力・出力を定義(P11)。説明(Description)を§5どおりに設定 | 作成済み(v6.0反映は未) |
| A4 | ツール接続: 子B=② / 子C=①③④(⑧は構築後) / 子D=①⑤⑥⑦ | 要確認(記号振り直し後の再点検) |
| A5 | 親: Instructions集v6.1 §A-2のスリム版Instructionsを貼付。**トピック11本**を§A-4のトピック仕様どおりに作成(手順は§C-0)。グローバル変数を作成。システムトピック「会話のリセット」をオフ | **未** |
...
6. [ ] A3: 子B〜EのInstructions v6.1反映、説明文設定
...
| 11 | グローバル変数はセッション内限定(MS公式仕様) | Phase 6で日をまたぐと、Phase 7以降で親の状態が空になる | セッションをまたぎ得るトピックの冒頭に復帰ステップ(SR/PS)を置く(Instructions集v6.1 §A-3)。**将来検討**: 案件状態(RITM・確定Mermaid・判定結果・確定版編集指示)をSharePointへ保存・復元するツール⑨を追加し、貼り付け運用を無くす |
| 12 | システムトピック「会話のリセット」が全グローバル変数を消す | Engの「最初から」等の発話で案件の状態が消える | PoC中はオフにする。オフにできない環境ではトリガーフレーズを確認し、Engはその表現を使わない |
