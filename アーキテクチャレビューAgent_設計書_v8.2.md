...
**名前の一致**: 生成オーケストレーションは子Agentを名前と説明文で選ぶため、親のInstructionsに書く子Agent名はCopilot Studio上の名前と完全一致させる。子の説明文には「親Agentからのみ呼び出される(Engとは直接会話しない)」を入れ、Engの発話から子が直接選ばれないようにする。各Agentの説明文・Instructions・ツール説明文・出力定義は Instructions集 v8.2 を正とする。
...
## 4. ツール一覧(名称と用途のみ。設計・入出力・参照先は「ツール一覧 v2.2」、⑦の詳細設計は「ツール設計書 v1.1」を参照)
...
| ③ | ホスティング判定基準取得 | ノックアウト要件一覧.md(列: No.(要因番号)/ 要因 / 具体的なケース / 判断区分 / 備考)の全文を返す。「DCS Hubで使えない条件」の一覧。CSPサービス起因の行は【構成図一覧】のサービスと突き合わせる材料 | ホスティング判定Agent | 実装済み(v2.1: 説明文のみ更新) |
...
| Input(ツール経由で子が読む) | Teamsスレッド(案件情報・ホスティング判断アプリの結果・利用者の回答)/ 回答済みアーキテクチャレビュー票Excel / 構成図チェック観点表 / ノックアウト要件一覧 / Salus Status一覧 / アーキテクチャレビュー票テンプレート / サービス提供対象リソース一覧 | Teams・SharePoint → ツール①〜⑤⑦⑨ → 子Agent |
...
| ホスティング判定Agent | 処理名(判定 / 再判定 / ナレッジ文案作成 / 登録実行)+RITM番号+【構成図一覧】+Engの訂正・補足(再判定時は回答回収結果も / 登録実行時は承認済み文案) | `result` | ## ホスティング判定結果(判定環境(+判定不能時は判定案)・判定理由(No.+判断区分)・スレッド由来CSP・■ノックアウト要件の照合結果(全行)・照合行数・Salus照合一覧・要人手確認・追加ヒアリング文)/ ナレッジ文案(16項目)/ 登録リンク |
...
| 子 | ①ツール①でスレッド(案件情報・ホスティング判断アプリの結果・利用者の回答)を読む → ②ツール③でノックアウト要件一覧(No./要因/具体的なケース/判断区分/備考)の全文を得て行数を控える。ツール④でSalus Status一覧を得る → ③【構成図一覧】が無ければ判定せず「先に一覧抽出を」と返す → ④ノックアウト要件一覧の全行を行順に照合: **(a)判断区分に具体的なサービス名・機能・構成パターンがある行(CSPサービス起因など。例: 特定のマネージドサービス / API Gatewayの種類 / VPN Gateway / CSP側のPublic IP / Auto Scaling)は【構成図一覧】の■サービス一覧・■NW情報・■注釈情報(とスレッドの利用予定)にそれがあるかで 該当(サービス名+ノードID)/ 非該当 を決める**(表記ゆれ・機能名での言及はAgentが柔軟に同一判断。理由と根拠を必ず書き、不確かなら「候補(要確認)」)/ (b)要件起因の行(NW要件・GCP利用・構成・アセット特性・SLA)はスレッドと■NW情報(CIDR・インターネット経路・Public Endpoint等)で 該当 / 非該当 / 判定不能 / (c)備考にAI向けの指示(「AIによる直接判断は行わず…」)や例外条件(「アウトバウンドのみの通信の場合はHub判断」「ReverseProxy経由でPrivate通信の場合はHub環境判断」)がある行は、条件を満たすことが読めれば非該当(Hub可)、満たさないことが読めれば該当、読めなければ「条件付き該当」として要人手確認+ヒアリング文 → ⑤判定(ノックアウト要件=DCS Hubで使えない条件): 該当(確定)が1行でもあれば DCS Disconnected / すべて非該当なら DCS Hub / 該当なしで条件付き該当・候補・判定不能があれば 判定不能(要確認行)+判定案 → ⑥Salus照合(一覧に無いものは「未照合」)→ ⑦「## ホスティング判定結果」(判定環境・判定理由(No.+判断区分)・■ノックアウト要件の照合結果(全行)・照合行数 NN/NN・Salus照合一覧・要人手確認・追加ヒアリング文)を返す。ナレッジは過去類似案件の参考提示のみ |
...
| 備考 | ノックアウト要件一覧.mdのNo.は要因番号(①〜⑦)で行ごとに一意ではないため、根拠は「No.+判断区分」で示す。行を一意に指せる「ID」列(KO-01…)の追加は任意の推奨(ツール一覧v2.1 ③参照) |
...
| 11 | 随時 | Eng | 資料のメンテナンス: 構成図チェック観点表(Excel。廃止行は廃止シートへ)、ノックアウト要件一覧md(AI向けの指示・条件は備考に書く)、Salus Status一覧md、アーキテクチャレビュー票テンプレートmd(CSP別)、サービス提供対象リソース一覧md、雛形xlsx | 観点の追加・変更はExcelの行で行い、Instructionsは変えない(設計原則9) |
...
| Instructions集 | v8.2 | 親・各子Agentの説明文・Instructions・ツール説明文・関連Agent・ナレッジ・出力定義・その他設定 | Copilot Studioへ貼り付ける正本。構成図Mermaid化AgentのInstructionsは現行のまま(貼付枠のみ) |
| ツール一覧 | v2.2 | ツール①〜⑨の名前・用途・設計(入力/処理/出力/参照先/状況) | ②の再設計(Excel直読み+適用行数。**推奨案・未承認**)、③の参照ファイル統一と判定の向き(v2.1/v2.2)、④の変更、⑨の新設を含む |
...
| 設計書 v7.1(Instructions制御版)/ v6.1(トピック制御版) | v7.1 / v6.1 | 旧構成(構成図修正Agentあり)。v6.1はトピック `T1a-1` の設計と変数一覧 | トピック制御へ戻す場合の参照。現行設計はv8.2が正 |
