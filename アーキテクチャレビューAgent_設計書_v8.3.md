...
Microsoft公式ドキュメント(表示されない場合はURLの `ja-jp` を `en-us` に読み替える):
...
| ホスティング判定Agent | `ホスティング判定Agent` | Teamsスレッドと【構成図一覧】を、ノックアウト要件一覧(ツール③)の全行と照合してDCS Hub / Disconnected を判定する(検算R5: 照合行数 ⇔ ツール③の全行数)。**判断区分に具体的なサービス名がある行(CSPサービス起因: DCS Hubで利用できないマネージドサービス等)は【構成図一覧】の■サービス一覧・■NW情報と突き合わせ**、要件起因の行(NW要件・GCP・構成・アセット特性・SLA)はスレッドと■NW情報で判定する。Salus Status(④)の照合。不足時は追加ヒアリング文。クローズ時はナレッジ文案作成、(ツール⑧がある場合)承認後の登録 | ① ③ ④(⑧) | 技術相談ナレッジ(Cons)※参考提示のみ | ★v8.1/v8.2。ノックアウト要件一覧=「DCS Hubで使えない条件」。1行でも該当→DCS Disconnected、すべて非該当→DCS Hub。サービス名の突き合わせは表記ゆれ(略称・正式名称・旧名称・機能名での言及)をAgentが柔軟に判断(根拠必須。不確かなら「候補(要確認)」)。備考のAI向け指示・例外条件に従う(条件付き該当は判定を確定せず要人手確認+ヒアリング文)。④は Salus Status一覧のみ。判定不能のときは親がヒアリング文を案内し、Engが環境を指定すればそれで進める |
...
**名前の一致**: 生成オーケストレーションは子Agentを名前と説明文で選ぶため、親のInstructionsに書く子Agent名はCopilot Studio上の名前と完全一致させる。子の説明文には「親Agentからのみ呼び出される(Engとは直接会話しない)」を入れ、Engの発話から子が直接選ばれないようにする。各Agentの説明文・Instructions・ツール説明文・出力定義は Instructions集 v8.3 を正とする。
...
graph LR
    ...
    SRC["参照先<br>Teams 共通_Engineering標準化(①⑦)<br>SharePoint AI連携用フォルダ(②③④⑤⑨=md表、⑥=雛形xlsx)<br>技術相談ナレッジList(⑧)"]:::data
...
## 4. ツール一覧(名称と用途のみ。設計・入出力・参照先は「ツール一覧 v2.3」、⑦の詳細設計は「ツール設計書 v1.1」を参照)

| No | ツール名 | 用途 | 持ち主 | 状況 |
|---|---|---|---|---|
| ① | Teamsスレッド内容取得 | RITM番号でスレッド(親投稿+全返信+添付一覧)を取得し、本文から判定したCSPを返す | ホスティング判定Agent / 構成図チェックAgent / アーキテクチャレビュー票作成Agent | 実装済み |
| ② | 構成図チェック観点取得 | 観点表(md表)から対象環境・CSP・1次/2次に該当する行を列位置で絞り、更新日時と適用行数を付けて返す | 構成図チェックAgent | 要改修(v2.3設計: 行数カウント+列位置フィルター。改修までは現行のmd全文版を使い、Agentが絞り込む) |
| ③ | ホスティング判定基準取得 | ノックアウト要件一覧.md(列: ID / No.(要因番号)/ 要因 / 具体的なケース / 判断区分 / 備考)の全文と全行数を返す。「DCS Hubで使えない条件」の一覧。CSPサービス起因の行は【構成図一覧】のサービスと突き合わせる材料。全行数は検算R5の分母 | ホスティング判定Agent | 要改修(行数カウントを追加。md側にID列を追加) |
| ④ | Salus Status一覧取得 | Salus_Status一覧.mdの全文と全行数を返す(サービス検証状況は返さない) | ホスティング判定Agent | 要改修(出力を1つに+行数カウント) |
| ⑤ | アーキテクチャレビュー票テンプレート取得 | CSP別テンプレートmd(No付き)の全文と全行数を返す。全行数は検算R2の分母 | アーキテクチャレビュー票作成Agent | 要改修(行数カウントを追加) |
...
| ⑨ | サービス提供対象リソース一覧取得 | サービス提供対象リソース一覧.md(利用者による動作検証必要○)の全文と全行数を返す | アーキテクチャレビュー票作成Agent | 新設・未実装(共通パターン) |
...
| Input(ツール経由で子が読む) | Teamsスレッド(案件情報・ホスティング判断アプリの結果・利用者の回答)/ 回答済みアーキテクチャレビュー票Excel / 参照データ(いずれもmd表。共通ルール集§6の約束に従う): 構成図チェック観点表 / ノックアウト要件一覧 / Salus Status一覧 / アーキテクチャレビュー票テンプレート / サービス提供対象リソース一覧 | Teams・SharePoint → ツール①〜⑤⑦⑨ → 子Agent(②③④⑤⑨は件数付きで返す) |
...
| ホスティング判定Agent | 処理名(判定 / 再判定 / ナレッジ文案作成 / 登録実行)+RITM番号+【構成図一覧】+Engの訂正・補足(再判定時は回答回収結果も / 登録実行時は承認済み文案) | `result` | ## ホスティング判定結果(判定環境(+判定不能時は判定案)・判定理由(ID+判断区分)・スレッド由来CSP・■ノックアウト要件の照合結果(全行)・検算R5・Salus照合一覧(件数付き)・要人手確認・追加ヒアリング文)/ ナレッジ文案(16項目)/ 登録リンク |
| 構成図チェックAgent | 処理名(1次 / 2次)+RITM番号+対象環境+スレッド由来CSP+確定Mermaid+注釈情報+【構成図一覧】+ホスティング判定結果(2次は回答回収結果も) | `result` | 【構成図チェック結果】RITM: / 処理: / 対象環境: / CSP: / 観点表の版(更新日時): / 適用行数: → ■観点別チェック / ■票で確認するもの / ■Eng目視確認 / ■要人手確認 / ■ヒアリング文 / 検算R1 |
...
flowchart TD
    ...
    G3{"情報充足?<br>判定不能ならヒアリング or Engが環境指定"}:::gate
...
| 子 | ①ツール①でスレッド(案件情報・ホスティング判断アプリの結果・利用者の回答)を読む → ②ツール③でノックアウト要件一覧(ID/No./要因/具体的なケース/判断区分/備考)の全文と「全行数: NN」(検算R5の分母)を得る。ツール④でSalus Status一覧を得る → ③【構成図一覧】が無ければ判定せず「先に一覧抽出を」と返す → ④ノックアウト要件一覧の全行を行順に照合: **(a)判断区分に具体的なサービス名・機能・構成パターンがある行(CSPサービス起因など。例: 特定のマネージドサービス / API Gatewayの種類 / VPN Gateway / CSP側のPublic IP / Auto Scaling)は【構成図一覧】の■サービス一覧・■NW情報・■注釈情報(とスレッドの利用予定)にそれがあるかで 該当(サービス名+ノードID)/ 非該当 を決める**(表記ゆれ・機能名での言及はAgentが柔軟に同一判断。理由と根拠を必ず書き、不確かなら「候補(要確認)」)/ (b)要件起因の行(NW要件・GCP利用・構成・アセット特性・SLA)はスレッドと■NW情報(CIDR・インターネット経路・Public Endpoint等)で 該当 / 非該当 / 判定不能 / (c)備考にAI向けの指示(「AIによる直接判断は行わず…」)や例外条件(「アウトバウンドのみの通信の場合はHub判断」「ReverseProxy経由でPrivate通信の場合はHub環境判断」)がある行は、条件を満たすことが読めれば非該当(Hub可)、満たさないことが読めれば該当、読めなければ「条件付き該当」として要人手確認+ヒアリング文 → ⑤判定(ノックアウト要件=DCS Hubで使えない条件): 該当(確定)が1行でもあれば DCS Disconnected / すべて非該当なら DCS Hub / 該当なしで条件付き該当・候補・判定不能があれば 判定不能(要確認行)+判定案 → ⑥Salus照合(■サービス一覧の全件。一覧に無いものは「未照合」)→ ⑦「## ホスティング判定結果」(判定環境・判定理由(ID+判断区分)・■ノックアウト要件の照合結果(全行)・検算R5 NN/NN・Salus照合一覧(NN件/サービス一覧NN件)・要人手確認・追加ヒアリング文)を返す。ナレッジは過去類似案件の参考提示のみ |
| 親 | `result` を保持し、判定環境を**対象環境**として保持。推定CSPとスレッド由来CSPが不一致なら「⚠ どちらが正しいか確認」を添える。判定が「判定不能」なら、ヒアリング文があればそれを先に案内し、あわせて「環境(Hub/Disconnected)を指定して進める場合は指定してください」と聞き、指定があれば対象環境として保持。**次の操作**: 情報充足 → 「構成図チェックして」/ 追加ヒアリングが必要 → ヒアリング文をスレッドへ投稿し、回答が反映されたら「再判断して」/ 一覧に訂正 → 「判定して。…」 |
| ループ | 「再判断して」で子はスレッドを再取得して判定し直す。親は判定結果と対象環境を最新版に置き換える |
| 人の作業 | ヒアリング文のスレッド投稿(Agentは投稿しない)、CSP不一致の確認、判定不能時の環境の指定(またはヒアリング回答待ち)、条件付き該当・候補の確認、再判断の指示 |
| 備考 | ノックアウト要件一覧.mdのNo.は要因番号(①〜⑦)で行ごとに一意ではないため、先頭に「ID」列(KO-01…)を追加し、根拠は「ID+判断区分」で示す(ID列が無い間は「No.+判断区分」)。検算R5の分母はツール③が返す全行数(Agentは数え直さない) |
...
| 備考 | 参照データはmd表に統一(v8.3)。ツール②は改修後、md表を列位置で絞って適用行数を返す。改修前は現行のmd全文版を使い、子が自分で絞り込む(R1の分母は子が数える。暫定) |
...
| 子(票作成) | ツール①で案件情報とCSPを確定 → ツール⑤でCSP別テンプレート(No付き)と「全行数: NN」(検算R2の分母)を取得 → ツール⑨でサービス提供対象リソース一覧を取得 → 材料を突き合わせて編集指示を作る: (a)ホスティング判定結果(判定環境・Salus照合)→ 該当項目の②添削 / ④不要 (b)【構成図チェック結果】の「票で確認するもの」→ テンプレート対象Noの行を②添削(依頼・確認内容を反映)、対応行が無ければ③追記 (c)申し送り事項: サービス一覧のうち⑨で「動作検証必要=○」のサービス → ③追記 (d)見直し時: 回答回収結果とEngの指摘を反映 → 検算R2(①流用+②添削+④不要 ⇔ ツール⑤が返した全行数。③追記は分母に含めない)→ 編集指示+アーキテクチャレビュー票mdを返す |
...
| 5 | フェーズ3 | Eng | CSP不一致(構成図由来 vs スレッド由来)の確認。判定不能のときの環境(Hub/Disconnected)の指定、または先にヒアリング。条件付き該当・候補(DCS Hubで利用できないサービスに当たるか)の確認。ヒアリング文のスレッド投稿と「再判断して」 | Agentはスレッドへ投稿しない |
...
| 11 | 随時 | Eng | 資料のメンテナンス(参照データはすべてmd表。共通ルール集§6の約束): 構成図チェック観点md(業務側のExcelを直したらmdへ変換して差し替え。廃止行は含めない)、ノックアウト要件一覧md(ID列。AI向けの指示・条件は備考に書く)、Salus Status一覧md、アーキテクチャレビュー票テンプレートmd(CSP別)、サービス提供対象リソース一覧md、雛形xlsx | 観点の追加・変更は表の行で行い、Instructionsは変えない(設計原則9)。行数はツールが数えるので書かない(設計原則11) |
...
| 文書 | 版 | 内容 | 位置づけ |
|---|---|---|---|
| 設計原則・共通ルール集 | v1.1 | 設計原則11か条 / 検算ルールR0〜R5 / 共通厳守事項C1〜C7 / 共通ブロック / 用語 / 参照データ(md表)の約束 | 全文書の単一ソース。本設計書には再掲しない |
| Instructions集 | v8.3 | 親・各子Agentの説明文・Instructions・ツール説明文・関連Agent・ナレッジ・出力定義・その他設定 | Copilot Studioへ貼り付ける正本。構成図Mermaid化AgentのInstructionsは現行のまま(貼付枠のみ) |
| ツール一覧 | v2.3 | ツール①〜⑨の名前・用途・設計(入力/処理/出力/参照先/状況)+参照データ取得ツールの共通パターン+md統一への移行手順 | ②③④⑤⑨の共通パターン化(md表+件数)、③のID列、④の変更、⑨の新設を含む |
| ツール設計書 | v1.1 | ⑦回答済みアーキテクチャレビュー票取得の各アクション設定・式・テスト観点(他ツールはv1.x時点の設計) | ⑦の詳細設計。他ツールはツール一覧v2.3が優先 |
...
| 設計書 v7.1(Instructions制御版)/ v6.1(トピック制御版) | v7.1 / v6.1 | 旧構成(構成図修正Agentあり)。v6.1はトピック `T1a-1` の設計と変数一覧 | トピック制御へ戻す場合の参照。現行設計はv8.3が正 |

**本版で採用したが未承認の推奨案(vetoがあれば差し戻す)**: 2次チェック時のヒアリング文は構成図チェックAgentがホスティング判定Agentの追加ヒアリング文と統合して1つの箇条書きにする(D-9)。(参照データのmd統一とツール②③④⑤⑨の共通パターン化は2026/09/14にEngが決定。ツール②のExcel直読み案は取り下げ)
